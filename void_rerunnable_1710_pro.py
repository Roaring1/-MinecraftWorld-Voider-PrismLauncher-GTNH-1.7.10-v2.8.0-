...
#   py voider_chunkmap_ui.py "C:\\...\\saves\\YourWorld"
#
# How to call from your main script:
#   from voider_chunkmap_ui import launch_chunk_selector
#   sel = launch_chunk_selector(world_path, initial_dim="DIM-1/region")
#   # sel is a dict keyed by dimtag; each value has "include", "exclude" sets of (x,z) pairs.
...
def _parse_loc(entry:int) -> Tuple[int,int]:
    # top 24 bits = sector offset, low 8 bits = sector count
...
def _iter_present_chunks_in_region(mca_path:Path) -> List[Tuple[int,int]]:
    ...
    with open(mca_path, "rb") as fp:
        ...
        for idx, entry in enumerate(offsets):
            ...
            if off == 0 or cnt == 0:
                continue
            # extra safety: confirm chunk has a non-zero length header
            try:
                fp.seek(off * SECTOR_BYTES)
                raw5 = fp.read(5)
                if len(raw5) < 5:
                    continue
                length = struct.unpack(">I", raw5[:4])[0]
                if length == 0:
                    continue
            except Exception:
                continue

            # convert idx (0..1023) => local chunk coords (0..31, 0..31)
            cx_local = idx % 32
            cz_local = idx // 32
...
def _find_region_dirs(world:Path) -> List[Path]:
    # any ".../region" folder that isn't a data-only subfolder
    out=[]
...
class Selection:
    ...
    def to_jsonable(self) -> Dict:
        ...
        return {"version": 1, "by_dim": out}

    def from_jsonable(self, data:Dict):
        self.by_dim.clear()
        if not data: return
        bd = data.get("by_dim", {})
        for dim, d in bd.items():
...
class ChunkCanvas(ttk.Frame):
    """
    Canvas wrapper for drawing the chunk grid and handling interaction.
    NOTE: we keep a stable `owner` reference to the window for theme + state.
    """
    def __init__(self, master, owner, world:Path, dimtag:str, present_chunks:Set[Tuple[int,int]], selection:Selection):
        super().__init__(master)
        self.owner = owner            # <- stable reference to ChunkMapWindow
        self.world = world
        self.dimtag = dimtag
        ...
        # compute extents from present chunks (fallback to small area)
        if present_chunks:
        ...
        # colors from the owning window
        self.bg = self.owner.colors["BG"]
        self.fg = self.owner.colors["Text"]
        self.grid_c    = "#384552" if not self.owner.is_light else "#d0d7de"
        self.exist_c   = "#244a61" if not self.owner.is_light else "#b6dfff"
        self.sel_inc_c = "#3fb950" if not self.owner.is_light else "#22863a"
        self.sel_exc_c = "#f85149" if not self.owner.is_light else "#d1242f"
        self.axis_c    = "#8b949e" if not self.owner.is_light else "#57606a"
        ...
        self.canvas.bind("<Button-5>", self._on_wheel_down)  # Linux X11
    ...
    def _on_wheel(self, ev):
        # use sign of delta (macOS deltas are small)
    ...
    def _on_lmb_drag(self, ev):
        ...
        if self.drag_rect is not None:
            ...
            self.drag_rect = self.canvas.create_rectangle(
                x1, y1, x2, y2, outline=self.owner.colors["ACCENT"], width=2, dash=(4,2)
            )
    ...
    def _on_lmb_up(self, ev):
        ...
        mode = self.owner.mode_var.get()  # include / exclude
        if (abs(cx2 - cx1) <= 0) and (abs(cz2 - cz1) <= 0):
            # single tile; Ctrl flips target set (Control mask = 0x0004 on Windows/X11)
            if (ev.state & 0x0004):
                mode = "exclude" if mode == "include" else "include"
            self.sel.add(self.dimtag, (cx1, cz1), mode)
        else:
            # Shift currently acts same as default (extend add-only)
...
class ChunkMapWindow(tk.Toplevel):
    def __init__(self, root, world:Path, colors:Dict[str,str], is_light:bool,
        ...
        if preload:
            self.selection.from_jsonable(preload)
        ...
        # top controls: dimension + selection mode + buttons
        ...
        self.btn_clear = ttk.Button(top, text="Clear", command=self._clear_current)
        self.btn_clear.pack(side="left", padx=(12,4))
        self.btn_load = ttk.Button(top, text="Load…", command=self._load_json)
        self.btn_load.pack(side="left", padx=4)
        self.btn_save = ttk.Button(top, text="Save", command=self._save_json)
        self.btn_save.pack(side="left", padx=4)
        self.btn_done = ttk.Button(top, text="Save & Close", style="Accent.TButton", command=self._done)
        self.btn_done.pack(side="right")

        # help text
        helpf = ttk.Frame(self); helpf.pack(fill="x", padx=10)
        help_lbl = ttk.Label(helpf, text="LMB: select (drag for box) • Ctrl+click toggles target • RMB/MMB: pan • Wheel: zoom • Include=green, Exclude=red")
        help_lbl.pack(anchor="w")

        # assemble dimension list and canvases
        self.dim_to_dir: Dict[str, Path] = {}
        dimtags=[]
        for rdir in dims:
            tag = str(rdir.relative_to(world)).replace("\\","/")
            dimtags.append(tag)
    ...
    def _on_dim_changed(self, *_):
        ...
        # pass 'self' as stable owner; DO NOT mutate widget.master
        self.current_canvas = ChunkCanvas(self.canvas_holder, self, self.world, dimtag, present, self.selection)
    ...
    def _load_json(self):
        ...
        try:
            ...
            self.selection.from_jsonable(data)
            if self.current_canvas: self.current_canvas._redraw_all()
    ...
    def _save_json(self):
        try:
            data = self.selection.to_jsonable()
    ...
    def _done(self):
        self._save_json()  # always write to world root as well
        self.destroy()

# ---------- public API ----------
def _detect_theme_colors() -> Tuple[Dict[str,str], bool]:
    # Simple light/dark palette; default to dark.
    DARK = {
        "BG": "#0d1117",
        "Panel": "#0f141b",
        "Text": "#e6edf3",
    ...
    # crude heuristic: keep DARK by default; wire system detection if desired
    try:
        return DARK, False
    except Exception:
        return DARK, False

def _discover_dims(world:Path) -> List[Path]:
    return _find_region_dirs(world)
...
def launch_chunk_selector(world_path:str,
                          ...
                          preload:Optional[Dict]=None) -> Dict:
    """Open the UI, block until closed, return JSON-like selection dict."""
    world = Path(world_path)
    if not (world/"level.dat").exists():
        raise SystemExit("level.dat not found — point to a valid world folder.")

    dims = _discover_dims(world)
    if not dims:
        raise SystemExit("No region folders found under this world.")
    ...
    try:
        ...
        pass
    root.withdraw()  # hide root; we use a Toplevel

    preload_data = _load_preexisting_selection(world, preload)

    win = ChunkMapWindow(root, world, colors, is_light, dims,
                         initial_dim=initial_dim or str(dims[0].relative_to(world)).replace("\\","/"),
    ...
    # read selection and ensure the JSON file exists on disk
    sel = win.selection.to_jsonable()
    try:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:\n  python voider_chunkmap_ui.py \"C:\\Path\\To\\World\" [DIMTAG]")
        sys.exit(1)
    world = sys.argv[1]
    dim = sys.argv[2] if len(sys.argv) > 2 else None
    sel = launch_chunk_selector(world, initial_dim=dim)
    # echo selection and path
